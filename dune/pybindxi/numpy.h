...
using is_pod_struct = all_of<
    ...
    && (__GLIBCXX__ < 20150422 || __GLIBCXX__ == 20150623 || __GLIBCXX__ == 20150626 || __GLIBCXX__ == 20160803)
...
public:
        ...
        handle base = handle())
  {
    ...
    auto tmp = reinterpret_steal<object>(api.PyArray_NewFromDescr_(api.PyArray_Type_,
                                                                   descr.release().ptr(),
                                                                   (int)ndim,
                                                                   // Use reinterpret_cast for PyPy on Windows (remove
                                                                   // if fixed, checked on 7.3.1)
  ...
  void resize(ShapeContainer new_shape, bool refcheck = true)
  {
    ...
    auto new_array = reinterpret_steal<object>(detail::npy_api::get().PyArray_Resize_(m_ptr, &d, int(refcheck), -1));
    if (!new_array)
      throw error_already_set();
...
{
  static constexpr auto name = _ < std::is_same<T, float>::value || std::is_same<T, const float>::value
