...
public:
  ...
  typedef typename std::iterator_traits<typename std::vector<Iterator>::iterator>::difference_type Size;
...
public:
  ...
  Partition(const RangedPartitioning& partitioning,
    ...
    , firstPartition_(firstPartition)
  ...
  Size size() const
  {
    return (lastPartition_ - firstPartition_) * partitioning_.stride_
           + (firstPartition_ < partitioning_.overflow_ ? partitioning_.overflow_ - firstPartition_ : 0);
  }
