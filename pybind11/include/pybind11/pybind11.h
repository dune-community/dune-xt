...
#  pragma warning disable 1334 // the "template" keyword used for syntactic disambiguation may only be used within a
                               // template
#  pragma warning disable 1682 // implicit conversion of a 64-bit integral type to a smaller integral type (potential
                               // portability problem)
...
#  pragma warning(disable : 4996) // warning C4996: The POSIX name for this item is deprecated. Instead, use the ISO C
...
#  pragma GCC diagnostic pop
  ...
  void
  initialize_generic(detail::function_record* rec, const char* text, const std::type_info* const* types, size_t args)
  {
    ...
    if (rec->sibling) {
      ...
      // Don't trigger for things like the default __init__, which are wrapper_descriptors that we are intentionally
      // replacing
...
public:
  ...
  PYBIND11_NOINLINE void add_object(const char* name, handle obj, bool overwrite = false)
  {
    if (!overwrite && hasattr(*this, name))
      pybind11_fail("Error during initialization: multiple incompatible definitions with name \"" + std::string(name)
                    + "\"");
...
protected:
  ...
  void install_buffer_funcs(buffer_info* (*get_buffer)(PyObject*, void*), void* get_buffer_data)
  {
    ...
    if (!type->ht_type.tp_as_buffer)
      pybind11_fail("To be able to register buffer protocol support for the type '" + std::string(tinfo->type->tp_name)
...
public:
  ...
  class_& def_static(const char* name_, Func&& f, const Extra&... extra)
  {
    ...
    cpp_function cf(std::forward<Func>(f), name(name_), scope(*this), sibling(getattr(*this, name_, none())), extra...);
...
private:
                          ...
                          const std::enable_shared_from_this<T>* /* dummy */)
  {
    try {
      auto sh =
          std::dynamic_pointer_cast<typename holder_type::element_type>(v_h.value_ptr<type>()->shared_from_this());
      if (sh) {
...
struct enum_base
{
  ...
  PYBIND11_NOINLINE void init(bool is_arithmetic, bool is_convertible)
  {
    ...
    m_base.attr("__doc__") = static_property(cpp_function([](handle arg) -> std::string {
    ...
    m_base.attr("__members__") = static_property(cpp_function([](handle arg) -> dict {
...
#undef PYBIND11_ENUM_OP_STRICT
    ...
    object getstate = cpp_function([](object arg) { return int_(arg); }, is_method(m_base));
...
inline function get_type_overload(const void* this_ptr, const detail::type_info* this_type, const char* name)
{
  ...
  if (frame && (std::string)str(frame->f_code->co_name) == name && frame->f_code->co_argcount > 0) {
    PyFrame_FastToLocals(frame);
    PyObject* self_caller = PyDict_GetItem(frame->f_locals, PyTuple_GET_ITEM(frame->f_code->co_varnames, 0));
