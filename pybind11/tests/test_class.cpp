...
TEST_SUBMODULE(class_, m)
{
  ...
  m.def("mismatched_holder_2", []() {
    ...
    py::class_<MismatchDerived2, std::shared_ptr<MismatchDerived2>, MismatchBase2>(mod, "MismatchDerived2");
  });
  ...
  struct ConvertibleFromUserType
  {
    ...
    ConvertibleFromUserType(UserType u)
      : i(u.value())
    {}
  };
  ...
  public:
    ...
    {
      PYBIND11_OVERLOAD(int, ProtectedB, foo, );
    }
  };
  ...
  py::class_<BogusImplicitConversion>(m, "BogusImplicitConversion").def(py::init<const BogusImplicitConversion&>());
...
#define CHECK_BASE(N)                                                                                                  \
  static_assert(std::is_same<typename DoesntBreak##N::type, BreaksBase<N>>::value, "DoesntBreak" #N " has wrong type!")
CHECK_BASE(1);
CHECK_BASE(2);
CHECK_BASE(3);
CHECK_BASE(4);
...
// CHECK_BROKEN(3);
//// Alias + 2 holders
// typedef py::class_<BreaksBase<-4>, std::unique_ptr<BreaksBase<-4>>, BreaksTramp<-4>, std::shared_ptr<BreaksBase<-4>>>
// Breaks4; CHECK_BROKEN(4);
