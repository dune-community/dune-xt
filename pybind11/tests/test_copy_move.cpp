...
TEST_SUBMODULE(copy_move_policies, m)
{
  ...
  m.def("move_pair", [](std::pair<MoveOnlyInt, MoveOrCopyInt> p) { return p.first.value + p.second.value; });
  ...
  m.def("move_copy_nested",
        [](std::pair<MoveOnlyInt,
                     std::pair<std::tuple<MoveOrCopyInt, CopyOnlyInt, std::tuple<MoveOnlyInt>>, MoveOrCopyInt>> x) {
