...
TEST_SUBMODULE(pickling, m)
{
  ...
  {
  public:
    Pickleable(const std::string& value)
      : m_value(value)
    {}
  ...
  py::class_<PickleableNew, Pickleable>(m, "PickleableNew")
      .def(py::init<std::string>())
      .def(py::pickle([](const PickleableNew& p) { return py::make_tuple(p.value(), p.extra1(), p.extra2()); },
                      [](py::tuple t) {
                        if (t.size() != 3)
                          throw std::runtime_error("Invalid state!");
  ...
  {
  public:
    PickleableWithDict(const std::string& value)
      : value(value)
    {}
  ...
  py::class_<PickleableWithDictNew, PickleableWithDict>(m, "PickleableWithDictNew")
      .def(py::init<std::string>())
      .def(py::pickle(
          [](py::object self) { return py::make_tuple(self.attr("value"), self.attr("extra"), self.attr("__dict__")); },
          [](const py::tuple& t) {
            if (t.size() != 3)
              throw std::runtime_error("Invalid state!");
