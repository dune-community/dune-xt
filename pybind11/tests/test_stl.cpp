...
TEST_SUBMODULE(stl, m)
{
  ...
  m.def("load_set",
        [](const std::set<std::string>& set) { return set.count("key1") && set.count("key2") && set.count("key3"); });
  ...
  py::class_<Issue1561Outer>(m, "Issue1561Outer").def(py::init<>()).def_readwrite("list", &Issue1561Outer::list);
}
