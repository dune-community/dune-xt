...
TEST_SUBMODULE(virtual_functions, m)
{
  ...
  py::class_<A2, PyA2>(m, "A2").def(py::init_alias<>()).def(py::init([](int) { return new PyA2(); })).def("f", &A2::f);
  ...
  public:
    ...
    std::string v;
    A a;
    explicit OverrideTest(const std::string& v)
      : v{v}
    {}
  ...
  public:
    ...
    A A_value() override
    {
      PYBIND11_OVERLOAD(A, OverrideTest, A_value);
    }
    A& A_ref() override
    {
      PYBIND11_OVERLOAD(A&, OverrideTest, A_ref);
    }
  };
...
void initialize_inherited_virtuals(py::module& m)
{
  ...
  py::class_<B_Tpl, A_Tpl, PyB_Tpl<>>(m, "B_Tpl").def(py::init<>()).def("lucky_number", &B_Tpl::lucky_number);
