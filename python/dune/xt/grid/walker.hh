...
public:
  ...
  bind(pybind11::module& m, const std::string& class_id = "walker", const std::string& grid_id = grid_name<G>::value())
  {
    ...
    c.def(
        "append",
        [](type& self, ElementFunctor<GV>& functor, const ElementFilter<GV>& filter) { self.append(functor, filter); },
        "element_functor"_a,
        "element_filter"_a = ApplyOn::AllElements<GV>());
    c.def("append",
          [](type& self, IntersectionFunctor<GV>& functor, const IntersectionFilter<GV>& filter) {
    ...
    c.def("append",
          ...
          "element_filter"_a = ApplyOn::AllElements<GV>());
    c.def("walk",
          [](type& self, const bool thread_parallel = false, const bool clear_functors_after_walk = true) {
